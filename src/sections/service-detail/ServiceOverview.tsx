      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-16 items-center mb-16">
          ...
          <div className="lg:w-1/2">
            <p className="text-gray-500 leading-relaxed italic">
              Our approach combines deep <Link href="/industries" className="text-[#7A1F5C] hover:underline font-semibold">industry knowledge</Link> with innovative <Link href="/services" className="text-[#7A1F5C] hover:underline font-semibold">recruitment methodologies</Link> to ensure we deliver not just candidates, but long-term value to your organization. We focus on cultural fit as much as technical excellence.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {FEATURE_CARDS.map((card, i) => (
            <Link key={i} href="/about" className="block h-[380px]">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="group relative overflow-hidden rounded-3xl bg-[#F5F0E8] h-full cursor-pointer shadow-sm hover:shadow-2xl transition-all duration-500"
              >
                {/* Image Area - Top */}
