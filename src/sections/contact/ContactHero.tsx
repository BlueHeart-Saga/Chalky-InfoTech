      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col items-center text-center max-w-5xl mx-auto">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-[#8A8A8A] text-sm sm:text-base md:text-xl leading-relaxed mb-12 max-w-3xl mx-auto font-medium px-4">
            Whether you're an ambitious organization looking to hire exceptional talent or a professional seeking your next career move — our specialist consultants are ready to assist you.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 w-full sm:w-auto px-4">
            <a href="#contact" className="group flex items-center justify-center gap-2 bg-[#7A1F5C] text-white px-8 sm:px-10 py-4 sm:py-5 rounded-2xl font-bold text-sm sm:text-base shadow-xl hover:shadow-[#7A1F5C]/40 hover:scale-105 transition-all duration-300">
              Send a Message <MessageSquare size={20} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </a>
            <a href="#offices" className="flex items-center justify-center gap-2 bg-[#F5F0E8] text-[#1A1A1A] px-8 sm:px-10 py-4 sm:py-5 rounded-2xl font-bold text-sm sm:text-base hover:bg-white transition-all duration-300 border border-[#EFE7DD] shadow-sm">
