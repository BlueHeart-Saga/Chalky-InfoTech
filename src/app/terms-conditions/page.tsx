...
const CATEGORY_ICONS: Record<string, any> = {
  "Agreement Fundamentals": Scale,
  "Website & Portal Usage": Globe,
  "Recruitment Service Terms": Briefcase,
  "Liability & Indemnification": ShieldAlert,
  "Dispute Resolution & Law": Gavel,
  "User Responsibilities": Users,
...
export default function TermsConditionsPage() {
  ...
  const filteredData = useMemo(() => {
    if (!searchQuery) return TERMS_CONDITIONS_DATA;
    
    return TERMS_CONDITIONS_DATA.map(cat => ({
      ...cat,
    ...
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <section className="pt-32 pb-20 bg-[#7A1F5C] text-white overflow-hidden relative">
        ...
        <div className="absolute bottom-8 right-8 z-20 hidden md:block">
          <DownloadPdfButton documentName="Terms & Conditions" variant="light" />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            Terms & <span className="text-[#F5A7CB]">Conditions</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Understand your rights and responsibilities when using Chalky Infotech's global recruitment services.
          </motion.p>
          ...
          >
            <div className="relative">
              ...
              <input 
                type="text"
                placeholder="Search legal terms (e.g. fees, liability, termination)..."
                value={searchQuery}
          ...
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <aside className="lg:col-span-1 hidden lg:block">
              <div className="sticky top-32 space-y-2">
                <h3 className="text-[#1A1A1A] font-bold text-sm uppercase tracking-wider mb-6 ml-4">Terms Sections</h3>
                {TERMS_CONDITIONS_DATA.map((cat, idx) => {
              ...
              {filteredData.length > 0 ? (
                <div className="space-y-16">
                  {filteredData.map((cat, catIdx) => {
                    const CategoryIcon = CATEGORY_ICONS[cat.category] || Scale;
                    return (
      ...
      <section className="pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-[#1A1A1A] rounded-[40px] p-8 md:p-16 text-center text-white relative overflow-hidden">
            <h2 className="text-3xl md:text-4xl font-semibold mb-6">Need legal clarity?</h2>
            <p className="text-white/60 mb-10 max-w-lg mx-auto">Our legal team is available to clarify any aspect of our service agreements.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href="/contact" className="bg-[#7A1F5C] px-8 py-4 rounded-full font-bold hover:bg-[#9D2877] transition-all flex items-center gap-2">
                Consult Legal <ChevronRight size={18} />
              </a>
              ...
              <div className="md:hidden mt-2">
                 <DownloadPdfButton documentName="Terms & Conditions" variant="light" className="px-8 py-4" />
              </div>
              <div className="hidden md:block">
                 <DownloadPdfButton documentName="Terms & Conditions" variant="light" className="py-4" />
              </div>
            </div>
          </div>
        </div>
