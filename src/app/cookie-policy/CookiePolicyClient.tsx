...
import { COOKIE_POLICY_DATA } from '@/constants/legalData';

const CATEGORY_ICONS: Record<string, any> = {
  "Fundamental Cookie Information": Info,
  "Strictly Necessary Cookies": ShieldCheck,
  "Analytical & Performance Cookies": Target,
  "Targeting & Marketing Cookies": Globe,
  "Functional & Preference Cookies": Settings,
  "Cookie Management & Control": Cookie,
  "Technical Specifications": Settings,
  "Regional & Global Standards": Globe
};
...
export default function CookiePolicyPage() {
  ...
  const filteredData = useMemo(() => {
    if (!searchQuery) return COOKIE_POLICY_DATA;
    
    return COOKIE_POLICY_DATA.map(cat => ({
      ...cat,
    ...
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <section className="pt-32 pb-20 bg-[#7A1F5C] text-white overflow-hidden relative">
        ...
        <div className="absolute bottom-8 right-8 z-20 hidden md:block">
          <DownloadPdfButton documentName="Cookie Policy" variant="light" />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            Cookie <span className="text-[#F5A7CB]">Policy</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            We use cookies to personalize your experience. Explore how we manage tracking and performance below.
          </motion.p>
          ...
          >
            <div className="relative">
              ...
              <input 
                type="text"
                placeholder="Search cookie types (e.g. analytics, marketing, settings)..."
                value={searchQuery}
          ...
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <aside className="lg:col-span-1 hidden lg:block">
              <div className="sticky top-32 space-y-2">
                <h3 className="text-[#1A1A1A] font-bold text-sm uppercase tracking-wider mb-6 ml-4">Cookie Categories</h3>
                {COOKIE_POLICY_DATA.map((cat, idx) => {
                  const Icon = CATEGORY_ICONS[cat.category] || Cookie;
                  return (
                    <button
                      key={idx}
                      onClick={() => {
              ...
              {filteredData.length > 0 ? (
                <div className="space-y-16">
                  {filteredData.map((cat, catIdx) => {
                    const CategoryIcon = CATEGORY_ICONS[cat.category] || Cookie;
                    return (
      ...
      <section className="pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-[#1A1A1A] rounded-[40px] p-8 md:p-16 text-center text-white relative overflow-hidden">
            <h2 className="text-3xl md:text-4xl font-semibold mb-6">Manage your preferences?</h2>
            <p className="text-white/60 mb-10 max-w-lg mx-auto">You can adjust your cookie settings at any time to control your privacy levels.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href="#" className="bg-[#7A1F5C] px-8 py-4 rounded-full font-bold hover:bg-[#9D2877] transition-all flex items-center gap-2">
                Cookie Settings <Settings size={18} />
              </a>
              <a href="/contact" className="bg-white/10 px-8 py-4 rounded-full font-bold hover:bg-white/20 transition-all border border-white/10">
                Contact Support
              </a>
            </div>
          </div>
