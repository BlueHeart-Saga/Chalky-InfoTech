...
import { VERIFICATION_DATA } from '@/constants/legalData';

const CATEGORY_ICONS: Record<string, any> = {
  "Verification Overview": Info,
  "DBS & Criminal Record Checks": ShieldCheck,
  "International Vetting Standards": Globe,
  "Reference & Integrity Vetting": UserCheck,
  "Education & Employment": Briefcase
};
...
export default function VerificationPage() {
  ...
  const filteredData = useMemo(() => {
    if (!searchQuery) return VERIFICATION_DATA;
    
    return VERIFICATION_DATA.map(cat => ({
      ...cat,
    ...
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <section className="pt-32 pb-20 bg-[#7A1F5C] text-white overflow-hidden relative">
        ...
        <div className="absolute bottom-8 right-8 z-20 hidden md:block">
          <DownloadPdfButton documentName="Background Verification" variant="light" />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            Background <span className="text-[#F5A7CB]">Verification</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Detailed insights into our rigorous candidate vetting process, from DBS checks to global reference validation.
          </motion.p>
          ...
          >
            <div className="relative">
              ...
              <input 
                type="text"
                placeholder="Search vetting types (e.g. DBS, credit check, education)..."
                value={searchQuery}
          ...
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <aside className="lg:col-span-1 hidden lg:block">
              <div className="sticky top-32 space-y-2">
                <h3 className="text-[#1A1A1A] font-bold text-sm uppercase tracking-wider mb-6 ml-4">Vetting Pillars</h3>
                {VERIFICATION_DATA.map((cat, idx) => {
            ...
            <div className="lg:col-span-3">
              {filteredData.length > 0 ? (
                ...
                <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-white">
                  <SearchIcon size={32} className="mx-auto mb-4 text-[#7A1F5C]/40" />
      ...
      <section className="pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-[#1A1A1A] rounded-[40px] p-8 md:p-16 text-center text-white relative overflow-hidden">
            <h2 className="text-3xl md:text-4xl font-semibold mb-6">Start Your Vetting?</h2>
            <p className="text-white/60 mb-10 max-w-lg mx-auto">Our security team ensures a fast, respectful, and fully compliant verification experience for all candidates.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href="/contact" className="bg-[#7A1F5C] px-8 py-4 rounded-full font-bold hover:bg-[#9D2877] transition-all flex items-center gap-2">
                Talk to Security <ChevronRight size={18} />
              </a>
              <a href="mailto:vetting@chalkyinfotech.com" className="bg-white/10 px-8 py-4 rounded-full font-bold hover:bg-white/20 transition-all border border-white/10">
                Email Vetting Team
              </a>
              <div className="md:hidden mt-2">
                 <DownloadPdfButton documentName="Background Verification" variant="light" className="px-8 py-4" />
              </div>
              <div className="hidden md:block">
                 <DownloadPdfButton documentName="Background Verification" variant="light" className="py-4" />
              </div>
            </div>
          </div>
        </div>
